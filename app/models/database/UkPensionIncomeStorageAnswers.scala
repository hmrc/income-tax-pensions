...
final case class UkPensionIncomeStorageAnswers(uKPensionIncomesQuestion: Boolean) extends StorageAnswers[UkPensionIncomeStorageAnswers] {
  def encrypted(implicit aesGCMCrypto: EncryptionService, textAndKey: TextAndKey): EncryptedStorageAnswers[UkPensionIncomeStorageAnswers] =
...
final case class EncryptedUkPensionIncomeStorageAnswers(uKPensionIncomesQuestion: EncryptedValue)
  ...
  protected[database] def unsafeDecrypted(implicit aesGCMCrypto: EncryptionService, textAndKey: TextAndKey): UkPensionIncomeStorageAnswers =

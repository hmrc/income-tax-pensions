  ...
  "FakeConnector" when {
    ...
    "host is External" should {
      ...
      "include all HeaderCarrier headers in the extraHeaders when the host is external" in {
        ...
        result.extraHeaders.contains(authorisation -> s"Bearer ${mockAppConfig.integrationFrameworkAuthorisationToken("1611")}") mustBe true
        result.extraHeaders.contains("Environment" -> mockAppConfig.integrationFrameworkEnvironment) mustBe true

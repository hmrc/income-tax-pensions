...
class ConnectorResponseInfoSpec extends AnyWordSpecLike {
  ...
  "logMessageWarnOn4xx" should {
    ...
    "return info on success with correlationId" in {
      assert(
        connectorInfo
          .copy(response = HttpResponse(200, "", Map(CorrelationIdHeaderKey -> List("just-for-test"))))
          .logResponseWarnOn4xx === Info("Connector [CorrelationId=just-for-test]: Response Received for GET /someurl. Response status: 200"))
    }

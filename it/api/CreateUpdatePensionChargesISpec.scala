...
class CreateUpdatePensionChargesISpec extends WiremockSpec with ScalaFutures {
  ...
  "create or update pension charges" should {
    ...
    "return 204 No Content on success" in new Setup {
      ...
      whenReady(
        buildClient(serviceUrl)
          ...
          .put(createUpdatePensionChargesPayload)) { result =>
    ...
    "return 400 if a there is an invalid header correlation id" in new Setup {
      ...
      val errorResponseBody: String = Json
        .toJson(DesErrorBodyModel("INVALID_CORRELATIONID", "Submission has not passed validation. Invalid Header parameter CorrelationId."))
        .toString()
    ...
    "return 400 if a there is an invalid payload" in new Setup {
      ...
      val errorResponseBody: String =
        Json.toJson(DesErrorBodyModel("INVALID_PAYLOAD", "Submission has not passed validation. Invalid payload.")).toString()
      ...
      whenReady(
        buildClient(serviceUrl)
          ...
          .put(Json.obj())) { result =>
        result.status mustBe BAD_REQUEST
        Json.parse(result.body) mustBe Json.obj("code" -> "INVALID_PAYLOAD", "reason" -> "Submission has not passed validation. Invalid payload.")
      }
    }
    ...
    "return 401" when {
      "user has no HMRC-MTD-IT enrolment" in new Setup {
        ...
        whenReady(
          buildClient(serviceUrl)
            .withHttpHeaders(requestHeaders: _*)
            .put(minimumRequestPayload)) { result =>
